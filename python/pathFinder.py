...
screen = pygame.display.set_mode((1366, 738))
...
window = Tk()
startXYLabel = Label(window, text='Start(x,y): ')
endXYLabel = Label(window, text='End(x,y): ')
startBox = Entry(window)
endBox = Entry(window)
var = IntVar()
showPath = ttk.Checkbutton(window, text='Show Steps :', onvalue=1, offvalue=0, variable=var)
submitButton = Button(window, text='Submit', command=onsubmit)

showPath.grid(columnspan=2, row=2)
submitButton.grid(columnspan=2, row=3)
endXYLabel.grid(row=1, pady=3)
endBox.grid(row=1, column=1, pady=3)
startBox.grid(row=0, column=1, pady=3)
startXYLabel.grid(row=0, pady=3)
...
def mousePress(x):
    t = x[0]
    w = x[1]
    g1 = t // (spot_height // cols)
    g2 = w // (spot_width // rows)
    acess = grid[g1][g2]
